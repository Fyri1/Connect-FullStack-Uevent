  ...
  return (
    <div className="w-full px-[5%]">
      <div data-popover-target={"error-" + id} data-popover-placement="right" data-popover-trigger="none" className="relative z-0 w-full mb-1 group">
        { children }
        <textarea onChange={inputChangeHandle} type={type} min={min} name="floating_email" id={id} rows="4" className="block py-2.5 px-7 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " value={data[id]}/>
        <label htmlFor="floating_email" className="px-10 pt-7 peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-00 transform -translate-y-[55px] scale-75 top-3 -z-10 origin-[0] peer-focus:left-0 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-95 peer-focus:-translate-y-[55px] peer-focus:-translate-x-8">{name}</label>
      </div>
