...
import apiClientRoute from '../../../../../routes/api/apiClientRoutes.js';

const Agreement = ({ setCompleteStep, completeStep }) => {
  const handelClick = async () => {
    try {
      const response = await $api.post(apiClientRoute.createOrganization(4), {
        isConfirmed: true,
      });
      ...
      <div className="p-8">
        <div className="bg-white p-4 rounded-lg shadow-xl py-8 mt-12">
          ...
          <button
            className=""
            onClick={() => {
              if (completeStep.step4.isComplete) {
                return;
              }
              handelClick();
            }}
